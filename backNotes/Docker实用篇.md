...
docker run --name nginx -v /opt/docker-volume/nginx/config/nginx.conf:/etc/nginx/nginx.conf -p 80:80 -d nginx
...
docker run \
 ...
 docker run -d --name mn -v html1:/usr/share/nginx/html -p 8080:80 nginx \
```
